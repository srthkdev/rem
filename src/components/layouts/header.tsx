...
export function Header() {
  ...
  useEffect(() => {
    if (user?.email && !user?.image) {
      ...
      const hash = Array.from(emailMd5).reduce(
      ...
      setProfileImage(
        `https://www.gravatar.com/avatar/${hash}?d=${encodeURIComponent(uiAvatarUrl)}`,
      );
    } else if (user?.image) {
      ...
      <div className="container flex h-16 items-center justify-between px-4 md:px-6 mx-auto">
        ...
        <div className="flex items-center">
          <Link href="/" className="flex items-baseline mr-4">
            <span className="font-[family-name:var(--font-work-sans)] text-2xl font-bold text-[#C96442]">
              REM
            </span>
          </Link>
        </div>
